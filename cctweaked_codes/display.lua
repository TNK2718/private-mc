...
function drawPx(x, y, color, mon)
  ...
  if response.getResponseCode() ~= 200 then error("Failed to download image, code: " .. tostring(code)) end

  local temp_filename = "temp.png"
  local file = io.open(temp_filename, "wb")
  file:write(response.readAll())
  file:close()

  -- Convert image to text
  local width, height = mon.getSize()
  local imageText = convertcc.process_image(temp_filename, width, height)
